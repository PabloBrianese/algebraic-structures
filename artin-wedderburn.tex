...
\begin{document}
  ...
  Tenemos \(m_{(r + s)} = m_r + m_s\) y \(m_{(r s)} = m_r m_s\) para todo par \(r, s \in R\).
  Consecuentemente, la aplicación \(m : R \rightarrow \End_E A\) definida por \(r \mapsto m_r\) es un homomorfismo de anillos.
  Llegamos a un nuevo anillo \(\mathcal{M} = \image(m)\).
  Decimos que \(\mathcal{M}\) es un \emph{anillo de multiplicadores}.
  ...
  Si \(A\) es simple, entonces \(D = \End_R A\) es un anillo de división.
  ...
  Su contenido es de caracter topológico, y marca que el anillo de multiplicadores \(\mathcal{M}\) separa, dentro del \(D\)-espacio vectorial \(A\), elementos \(a \in A\) y subespacios \(V\) de dimensión finita.

  \begin{lemma}
    \label{lemma:anihilatorLemma}
    Sea \(R\) un anillo, \(A\) un \(R\)-módulo simple y \(\mathcal{M}\) el anillo de multiplicadores.
    Consideramos \(A\) como un espacio vectorial sobre el anillo de división \(D = \End_R A\).
    Si \(V\) es un subespacio finito-dimensional del \(D\)-espacio vectorial \(A\) y \(a \in A \setminus V\), entonces existe \(T \in \mathcal{M}\) tal que \(T(a) \neq 0\) y \(T(V) = 0\).
  \end{lemma}
  \begin{proof}
    La prueba es por inducción sobre \(n = \dim_D V\).
    Comenzamos por el caso base.
    Si \(n = 0\), entonces \(V = 0\) y \(a \neq 0\).
    Porque \(A\) es simple, \(a \neq 0\) implica \(R a = A\) (Proposición \ref{proposition:simpleModulesAreCiclic}).
  ...
  \begin{theorem}[de Densidad de Jacobson]
    ...
    Considerar \(A\) como espacio vectorial sobre el anillo de división \(D = \Hom_R(A, A)\).
