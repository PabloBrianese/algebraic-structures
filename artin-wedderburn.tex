...
\begin{document}
  ...
  \end{theorem}
  \begin{proof}
    Si \(J\) es un ideal de \(S\), entonces, por el teorema \ref{theorem:matrixRingIdeals}, \(J\) es el anillo formado por todas las matrices \(n \times n\) sobre \(I\) para algún ideal \(I\) en \(D\).
  ...
  \begin{proof}
    ...
    Pero \(R\) es simple: no tiene ideales bilaterales propios, por lo cual \(I = R\) o \(I = 0\).
    ...
    Supongamos, para llegar a un absurdo, que \(\mathcal{A}(J) = 0\).
    Entonces \(\mathcal{A}(J) = R\) y \(R J = 0\).
    Pero existen elementos \(u \in J\) distintos de 0.
    Cada uno de estos \(u\) nonulos pertenece al ideal \(I\) que estudiamos al inicio de la prueba.
    Siendo \(I = 0\), llegamos a una contradicción.
    Por lo tanto \(\mathcal{A}(J) = 0\) y \(R J \neq 0\).
    En conclusión, \(J\) es un \(R\)--módulo simple y fiel. 
    El anillo \(R\) es primitivo.
  \end{proof}
  ...
  \begin{proof}
    ...
    La existencia de esta cadena lleva a una contradicción.
    Luego \(\dim_D V\) es finita.

    Recíprocamente, supongamos que \(\dim_D V\) es finita.
